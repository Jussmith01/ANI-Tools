...
M   = 0.1 # Max error per atom in kcal/mol
...
wkdir = '/home/jsmith48/scratch/auto_dim_al/modeldim/ANI-1x-DIM-0001/'
iptfile = '/home/jsmith48/scratch/auto_dim_al/modeldim/inputtrain.ipt'
saefile = '/home/jsmith48/scratch/auto_dim_al/modeldim/sae_linfit.dat'
cstfile = '/home/jsmith48/scratch/auto_dim_al/modeldim/rHCNO-5.2R_16-4.0A_a4-8.params'
...
dmrparams = {#'mdselect' : [(400,0),(60,2),(40,3),(5,4)],
             ...
             #'N' : 20,
             'maxNa' : 40,
             'T' : 500.0, # running temp 
             'L' : 30.0, # box length
             'V' : 0.02, # Random init velocities 
             'dt' : 0.25, # MD time step
             'Nm' : 220, # Molecules to embed
             'Nr' : 40, # Number of total boxes to embed and test
             'Ni' : 50, # Number of steps to run the dynamics before fragmenting
             'Ns' : 50, # Number of MD blocks
             'sig': M,
            }
...
gcmddict = {'edgepad': 0.8, # padding on the box edge
            ...
            'MolHigh': 920, #High number of molecules
...
for i in N:
    ...
    if i >= 0:
        ...
        netdir = wkdir + 'ANI-1x-DIM-0001.00' + str(i).zfill(2) + '/'
    ...
    ast.generateQMdata(hostname, username, swkdir, ldtdir, datdir + str(i+1).zfill(2), h5stor, mae, jtime, max_jobs=75)
