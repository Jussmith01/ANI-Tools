...
cachet = [cg('_train', saef, store_dir + str(r) + '/',forcet,chargt,False) for r in range(N)]
cachev = [cg('_valid', saef, store_dir + str(r) + '/',forcet,chargt,False) for r in range(N)]
...
for f,fn in enumerate(h5files):
    ...
    for c, data in enumerate(adl):
        if True:
            ...
            index = np.where(Mv > 100.0)[0]
            indexk = np.where(Mv <= 100.0)[0]
            #if index.size > 0:
                #print(Mv[index])
            ...
            if (set(S).issubset(['C', 'N', 'O', 'H'])):
                ...
                for i,t,v,te in zip(range(N), cachet, cachev, testh5):
                    ...
                    #F_t = np.empty([0, F.shape[1], 3], dtype=np.float32)
                    Q_t = np.array(np.concatenate([Q[s] for j, s in enumerate(split) if j != i]), order='C', dtype=np.float32)
                    E_t = np.array(np.concatenate([E[s] for j, s in enumerate(split) if j != i]), order='C', dtype=np.float64)

                    if E_t.shape[0] != 0:
                        t.insertdata(X_t, F_t, Q_t, E_t, list(S))
                    ...
                    if tv_split[0].size > 0:
                        ...
                        #F_v = np.empty([0, F.shape[1], 3], dtype=np.float32)

                        Q_v = np.array(Q[tv_split[0]], order='C', dtype=np.float32)
                        E_v = np.array(E[tv_split[0]], order='C', dtype=np.float64)
                        if E_v.shape[0] != 0:
                            v.insertdata(X_v, F_v, Q_v, E_v, list(S))
                    ...
                    if tv_split[1].size > 0:
                        ...
                        Q_te = np.array(Q[tv_split[1]], order='C', dtype=np.float32)
                        E_te = np.array(E[tv_split[1]], order='C', dtype=np.float64)
                        if E_te.shape[0] != 0:
                            te.store_data(Pn, coordinates=X_te, forces=F_te, hirshfeld=Q_te, energies=E_te, species=list(S))
